...
class Station(Producer):
    ...
    logger.info(f"loading {Path(__file__).parents[0]}/schemas/arrival_key.json")
    key_schema = avro.load(f"{Path(__file__).parents[0]}/schemas/arrival_key.json")

    #
    # TODO: Define this value schema in `schemas/station_value.json, then uncomment the below
    #
    logger.info(f"loading {Path(__file__).parents[0]}/schemas/arrival_value.json")
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        #
        #
        logger.info(f"producing {self.topic_name} for station {self.station_id} train {train.train_id} with status {train.status.name} and color {self.color.name}")
        ...
        logger.info(f"produced {self.topic_name} for station {self.station_id} train {train.train_id}")
